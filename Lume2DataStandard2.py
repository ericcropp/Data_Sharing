...
for i in range(len(impact_filenames['impact_archive'])):
    ...
    for key, value in I.output['particles'].items():
        if key != 'final_particles' and key != 'initial_particles':
            D.add_output(location=key, datum=value, attrs={}, datum_name=key, datum_type='distribution')
    # Add final_particles as a distribution output
    D.add_output(location=I.particles['final_particles']['mean_z'], datum=I.particles['final_particles'], attrs={}, datum_name='final_particles', datum_type='distribution')
